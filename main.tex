...
\begin{document}
	...
	Aplikace pracuje se čtyřmi pevně danými tabulkami: soutěže, soutěžící, porota a harmonogram. Definice těchto tabulek a mapovacích polí je centralizována v registru \texttt{tables.py} (\texttt{TableKey}, \texttt{TableSpec}, \texttt{TableMappingSchema}).
	...
	Doménový model je implementován třídami \texttt{Competition}, \texttt{Performance}, \texttt{Competitor}, \texttt{JuryMember}. Každá entita provádí základní validaci v metodě \texttt{\_\_post\_init\_\_} (např. kladné ID, neprázdná jména, konzistence délky a času výkonu).
	
	\subsection{Centrální repozitář}
	Po parsování jsou entity ukládány do \texttt{ScheduleRepository}. Repozitář drží:
	\begin{itemize}
		\item soutěže podle ID,
		\item vystoupení podle ID soutěže,
		\item množinu soutěžících,
		\item množinu členů poroty.
	\end{itemize}
	
	Dále poskytuje pomocné dotazy, například seznam vystoupení člověka (\texttt{list\_assignments\_of\_human}) seřazený podle času.
