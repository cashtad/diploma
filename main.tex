...
\begin{document}
	...
	Všechny tyto soutěže a jejich jednotlivá kola je nutné správně zařadit do časového harmonogramu tak, aby byl přehledný a srozumitelný nejen pro vedoucí soutěže, ale také pro soutěžící, jejich doprovod a členy poroty. Harmonogram musí umožnit účastníkům orientaci v průběhu dne a zároveň zajistit plynulý průběh celé soutěže. Chyby v organizaci nebo v časovém rozvržení mohou vést k výrazným problémům, jako jsou časové skluzy, nespokojenost účastníků nebo snížení celkové kvality soutěže.
	
	
	\section{Struktura soutěže a základní entity}
	Z pohledu tvorby časového harmonogramu lze taneční soutěž popsat pomocí několika základních entit. Pro účely této práce jsou klíčové zejména tři hlavní entity: taneční soutěže, účastníci a členové poroty.
	...
	Součástí tohoto systému je možnost exportu dat do tabulkového formátu, konkrétně ve formátu souborů typu XLSX. Tyto exportované tabulky představují hlavní vstupní data pro další práci s harmonogramem soutěže. Struktura těchto tabulek je pevně daná a nemění se mezi jednotlivými soutěžemi. Vedoucí soutěže nemají možnost tuto strukturu upravovat ani rozšiřovat.
	
	Přestože exportovaná data obsahují všechny základní informace o soutěžích, účastnících a porotcích, neposkytují přímou podporu pro tvorbu a kontrolu časového harmonogramu. Práce s těmito tabulkami je proto časově náročná a vyžaduje manuální analýzu velkého množství informací se snadným zanesením chyb.
	
	\section{Problém tvorby časového harmonogramu}
	Tvorba časového harmonogramu taneční soutěže obvykle začíná přibližně až dva týdny před samotným konáním akce. V této fázi vedoucí soutěže rozdělují jednotlivé soutěže do časových bloků a přiřazují členy poroty ke konkrétním soutěžím. Jedná se o velmi náročný proces, který vyžaduje vysokou míru soustředění a pečlivosti.
	
	Při sestavování harmonogramu je nutné zohlednit velké množství faktorů, které nejsou na první pohled zřejmé. Tyto faktory je třeba sledovat napříč několika tabulkami, jejichž struktura není vždy intuitivní a přehledná. Vedoucí musí neustále kontrolovat možné kolize, časové rozestupy a vazby mezi jednotlivými soutěžemi, účastníky a porotci.
	...
	Při tvorbě časového harmonogramu musí vedoucí dodržovat velké množství časových a organizačních omezení. Některá z těchto omezení jsou zřejmá, například skutečnost, že dvě soutěže nemohou probíhat současně v případě, že je k dispozici pouze jedna soutěžní plocha. Existují však i omezení, která lze identifikovat pouze podrobnou analýzou dat a vzájemných vazeb mezi jednotlivými entitami, jako například skutečnost, že ve více paralelně probíhajících soutěžích nemohou vystupovat stejní soutěžící.
	...
	Dalším důležitým omezením je zabránění časovým kolizím, kdy by účastník nebo porotce byl přiřazen ke dvěma soutěžím probíhajícím ve stejném čase. Sledování a dodržování všech těchto omezení je při častých změnách harmonogramu velmi časově náročné, a to i pro zkušené vedoucí soutěže.

	\section{Důsledky porušení harmonogramu}
	Nedodržení časového harmonogramu nebo porušení výše uvedených omezení může mít řadu negativních důsledků. Mezi nejčastější problémy patří časové zpoždění soutěže, které se může postupně kumulovat během celého dne. To může vést ke stresu soutěžících, porotců i vedoucích soutěže.

	Dalším důsledkem může být fyzická únava účastníků, zejména v případech, kdy nejsou dodrženy dostatečné přestávky mezi vystoupeními. Únava může negativně ovlivnit sportovní výkon soutěžících i kvalitu hodnocení ze strany poroty. V extrémních případech může špatně sestavený harmonogram vést ke snížení celkové úrovně soutěže, vzniku zmatku, nepříjemné atmosféry a k negativní zpětné vazbě ze strany účastníků.


	\section{Shrnutí analytické části}
	Z provedené analýzy vyplývá, že tvorba časového harmonogramu taneční soutěže představuje komplexní a časově náročný proces, který zahrnuje práci s velkým množstvím dat a dodržování celé řady omezení. Manuální kontrola všech vazeb a pravidel je pro vedoucí soutěže značně zatěžující a náchylná k chybám.

	Z těchto důvodů se jeví jako vhodné využití softwarového nástroje, který by vedoucím pomohl s kontrolou již vytvořeného harmonogramu. Takový nástroj může sloužit jako podpůrný prostředek, který umožní rychlou identifikaci problematických míst v harmonogramu a přispěje ke zvýšení efektivity a kvality organizace taneční soutěže.

	\chapter{Analýza požadavků a návrh systému}

	\section{Cíl práce}

	Hlavním cílem této práce je návrh a implementace softwarového nástroje, který usnadní vedoucím soutěže tanečních soutěží proces kontroly časového harmonogramu. Výsledná aplikace má sloužit jako podpůrný nástroj pro ověřování správnosti již vytvořeného harmonogramu a pro identifikaci jeho slabých míst.
	...
	\begin{itemize}
		\item analyzovat vstupní data a jejich strukturu,
		\item navrhnout systém pro kontrolu porušení harmonogramu,
		\item implementovat mechanismus hodnocení nalezených problémů,
		\item zajistit přehlednou prezentaci výsledků uživateli.
	\end{itemize}
	...
	Navrhovaný systém musí splňovat následující funkční požadavky, které definují jeho základní chování a schopnosti. Systém musí být schopen:
	\begin{itemize}
		\item načíst vstupní tabulky obsahující informace o soutěžích,
		\item načíst tabulky s údaji o účastnících a jejich registracích,
		\item načíst tabulku s rozdělením poroty k jednotlivým soutěžím,
		\item načíst tabulku s časovým harmonogramem soutěže,
		\item propojit data z jednotlivých tabulek na základě společných identifikátorů nebo názvů,
		\item provést kontrolu harmonogramu podle předem definovaných pravidel,
		\item identifikovat porušení těchto pravidel,
		\item zobrazit výsledky kontroly v přehledné a srozumitelné formě.
	\end{itemize}
	...
	Navrhovaný systém pracuje se strukturovanými vstupními daty ve formě tabulek. Tyto tabulky jsou generovány externími systémy nebo vytvářeny ručně vedoucími soutěže. Každá tabulka má v systému jasně definovanou roli a obsahuje specifický typ informací potřebných pro kontrolu harmonogramu.
	
	Na vstupu aplikace přijímá celkem čtyři tabulky:
	\begin{itemize}
		\item první tabulka obsahuje informace o soutěžích. Z této tabulky je možné získat základní údaje o jednotlivých soutěžích, například jejich název, typ soutěže, soutěžní ligu nebo jednoznačný identifikátor soutěže. Tato tabulka slouží jako základní referenční zdroj pro ostatní vstupní data,
		\item druhá tabulka obsahuje informace o účastnících soutěže. V této tabulce je uvedeno, na které soutěže jsou jednotliví účastníci registrováni. Na základě těchto údajů je možné sledovat účast konkrétních osob v průběhu celého harmonogramu a ověřovat dodržení časových omezení mezi jejich vystoupeními,
		\item třetí tabulka popisuje rozdělení poroty k jednotlivým soutěžím. Obsahuje informace o tom, kteří porotci jsou přiřazeni ke konkrétním soutěžím nebo jejich částem. Tato data jsou důležitá zejména pro kontrolu vytížení poroty a případných časových konfliktů,
		\item čtvrtou a nejdůležitější vstupní tabulkou je tabulka s časovým harmonogramem soutěže. Tato tabulka obsahuje informace o časech začátků jednotlivých kol soutěží. Jednotlivé položky harmonogramu musí být jednoznačně propojeny se soutěžemi uvedenými v tabulce soutěží, a to buď pomocí identifikátoru, nebo shodného názvu. Bez tohoto propojení není možné provést správnou analýzu harmonogramu.
	\end{itemize}
	...
	Každé kontrolní pravidlo je v systému implementováno samostatně a je možné jej při kontrole zapnout nebo vypnout podle aktuálních potřeb vedoucího soutěže. Uživatel má také možnost upravit vybrané parametry pravidel, například minimální dobu odpočinku mezi vystoupeními nebo maximální přípustnou délku čekání mezi soutěžemi. Díky tomu lze systém přizpůsobit různým typům soutěží a jejich specifickým požadavkům.
	
	Nalezená porušení pravidel jsou v systému rozdělena do několika kategorií podle jejich závažnosti. Rozlišují se méně závažná porušení, která mají spíše informativní charakter, a závažná nebo kritická porušení, která mohou výrazně negativně ovlivnit průběh soutěže. Toto rozdělení umožňuje vedoucím soutěže lépe prioritizovat problémy, které je nutné řešit jako první.
	...
	Výstupem hodnocení je přehledný report, který obsahuje celkový počet nalezených porušení, jejich rozdělení podle jednotlivých kategorií a seznam porušení pro každé kontrolní pravidlo. U každého problému je uvedeno jeho umístění v harmonogramu, aby mohl vedoucí soutěže snadno identifikovat konkrétní část rozvrhu, která vyžaduje pozornost.
	
	Navržený způsob hodnocení slouží především jako podpůrný nástroj pro vedoucí soutěže. Poskytuje jim strukturovaný pohled na harmonogram a pomáhá snížit riziko přehlédnutí organizačních problémů, které by mohly negativně ovlivnit průběh soutěže.
	...
	Závěrem byl popsán způsob hodnocení harmonogramu, který umožňuje systematickou klasifikaci nalezených porušení a poskytuje vedoucím soutěže přehledné a srozumitelné výsledky kontroly.
	...
	% TODO: Kapitola zatím pokrývá doménový kontext a praktické problémy.
	% Chybí však formalizace problému, požadavky na aplikaci a porovnání přístupů.
	% Tyto části budou doplněny níže jako nové sekce.
	...
	První entitou jsou samotné taneční soutěže. Každá soutěž je definována několika základními atributy, mezi které patří věková kategorie účastníků, typ tance, úroveň výkonnosti a soutěžní liga. Jednotlivé soutěže mohou mít různý počet kol, který je obvykle stanoven na základě počtu přihlášených účastníků.

	Druhou důležitou entitou jsou účastníci soutěže. Účastníkem bývá typicky jednotlivec nebo taneční pár. V praxi se často stává, že jeden soutěžící, zejména v dětských a juniorských kategoriích, vystupuje v deseti nebo i více soutěžích. Mezi jednotlivými vystoupeními může být navíc nutná výměna tanečního kostýmu.

	Třetí entitou jsou členové poroty. Podobně jako soutěžící, i členové poroty jsou během dne zapojeni do více soutěží, a při sestavování harmonogramu je proto nutné zohlednit jejich potřebu odpočinku i efektivní využití času.
	
	% добавить четвертую энтиту в виде выступления. Время, тип, отношение к соревнованию

	\subsection{Význam harmonogramu pro průběh akce}
	Všechny soutěže a jejich jednotlivá kola je nutné správně zařadit do časového harmonogramu tak, aby byl přehledný a srozumitelný nejen pro vedoucí soutěže, ale také pro soutěžící, jejich doprovod a členy poroty. Harmonogram musí umožnit účastníkům orientaci v průběhu dne a zároveň zajistit plynulý průběh celé soutěže.

	Chyby v organizaci nebo v časovém rozvržení mohou vést k výrazným problémům, jako jsou časové skluzy, nespokojenost účastníků nebo snížení celkové kvality soutěže.

	\section{Problém tvorby a kontroly harmonogramu}

	\subsection{Typické konfliktní situace}
	Při tvorbě časového harmonogramu musí vedoucí dodržovat velké množství časových a organizačních omezení. Některá z těchto omezení jsou zřejmá, například skutečnost, že dvě soutěže nemohou probíhat současně v případě, že je k dispozici pouze jedna soutěžní plocha. Existují však i omezení, která lze identifikovat pouze podrobnou analýzou dat a vzájemných vazeb mezi jednotlivými entitami, například skutečnost, že ve více paralelně probíhajících soutěžích nemohou vystupovat stejní soutěžící.

	Mezi klíčová omezení patří zejména potřeba zajistit, aby soutěžící nevystupovali po dlouhou dobu bez přestávky na odpočinek. Dále je nutné zajistit dostatečný časový prostor pro převlékání kostýmů při změně typu tance. Harmonogram by měl být sestaven tak, aby soutěžící i členové poroty nemuseli na své další zapojení do soutěže čekat nepřiměřeně dlouhou dobu.

	Dalším důležitým omezením je zabránění časovým kolizím, kdy by účastník nebo porotce byl přiřazen ke dvěma soutěžím probíhajícím ve stejném čase.

	\subsection{Limity ruční kontroly}
	Tvorba časového harmonogramu taneční soutěže obvykle začíná přibližně dva týdny před samotným konáním akce. V této fázi vedoucí soutěže rozdělují jednotlivé soutěže do časových bloků a přiřazují členy poroty ke konkrétním soutěžím. Jedná se o velmi náročný proces, který vyžaduje vysokou míru soustředění a pečlivosti.
	...
	% TODO: Popsat reprezentaci času, intervalů a návazností mezi vystoupeními.
	% Definovat pojem časového intervalu a detekci překryvu.

	\subsection{Definice omezení a porušení}
	% TODO: Formálně definovat pravidla (kolize soutěžících, poroty,
	% minimální přestávky, převlékání apod.) a pojem porušení pravidla.
	% TODO: Doplnit klasifikaci omezení (hard vs. soft constraints).
	...
	Součástí tohoto systému je možnost exportu dat do tabulkového formátu, konkrétně ve formátu souborů typu XLSX. Tyto exportované tabulky představují hlavní vstupní data pro další práci s harmonogramem soutěže.

	% TODO: Doplnit detailní popis konkrétních tabulek a klíčových sloupců
	% (soutěže, soutěžící, porota, rozvrh) včetně vazeb přes identifikátory.

	\subsection{Rizika nekonzistence a nekvality dat}
	Přestože exportovaná data obsahují základní informace o soutěžích, účastnících a porotcích, neposkytují přímou podporu pro tvorbu a kontrolu časového harmonogramu. Práce s těmito tabulkami je proto časově náročná a vyžaduje manuální analýzu velkého množství informací se snadným zanesením chyb.

	% TODO: Doplnit typické datové chyby (chybějící hodnoty,
	% neplatné reference, duplicity) a jejich dopad na analýzu. указать то, что таблица расписания делается как попало и не имеет строгого формата.

	\section{Požadavky na aplikaci}

	\subsection{Funkční požadavky}
	% TODO: Načtení dat, mapování sloupců, validace vstupů,
	% pravidlová analýza harmonogramu, generování HTML reportu,
	% zobrazení porušení nad tabulkou rozvrhu.
	
	\subsection{Nefunkční požadavky}
	% TODO: Použitelnost pro organizátory, stabilita, rozšiřitelnost pravidel,
	% srozumitelnost výstupu, přiměřené nároky na běžný HW.

	\subsection{Rozsah a hranice řešení}
	% TODO: Zdůraznit, že aplikace harmonogram neoptimalizuje,
	% ale validuje existující plán; neprovádí editaci dat v externím systému.

	\section{Přístupy k řešení a existující praxe}

	\subsection{Hodnoticí kritéria porovnání}
	% TODO: Definovat kritéria (přesnost, transparentnost,
	% časová náročnost, rozšiřitelnost).

	\subsection{Manuální a tabulkové postupy}
	V současné praxi převažuje ruční kontrola tabulek, případně jednoduché tabulkové postupy. Tyto přístupy jsou použitelné u jednodušších scénářů, ale při vyšší složitosti harmonogramu rychle narůstá časová náročnost i riziko přehlédnutí konfliktů.

	\subsection{Pravidlově orientovaný přístup}
	Na základě předchozí analýzy se jeví jako vhodné využití softwarového nástroje, který pomáhá automaticky vyhodnocovat vazby mezi entitami a kontrolovat dodržení definovaných pravidel. Pravidlový přístup umožňuje systematickou, opakovatelnou a transparentní kontrolu harmonogramu.

	\subsection{Zdůvodnění volby výsledného přístupu}
	% TODO: Obhájit volbu specializovaného desktopového nástroje
	% s pravidlovou analýzou pro kontrolu již vytvořeného harmonogramu.
	
	\section{Shrnutí}
	Z provedené analýzy vyplývá, že tvorba časového harmonogramu taneční soutěže představuje komplexní a časově náročný proces, který zahrnuje práci s velkým množstvím dat a dodržování celé řady omezení. Manuální kontrola všech vazeb a pravidel je pro vedoucí soutěže značně zatěžující a náchylná k chybám.

	Z těchto důvodů se jeví jako vhodné využití softwarového nástroje, který vedoucím pomůže s kontrolou již vytvořeného harmonogramu. Takový nástroj může sloužit jako podpůrný prostředek, který umožní rychlou identifikaci problematických míst v harmonogramu a přispěje ke zvýšení efektivity a kvality organizace taneční soutěže.
